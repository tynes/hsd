...
class TX extends bio.Struct {
  ...
  /**
   * Compute the StandardTemplateHash according to BIP0119.
   * @param {Number} index
   * @returns {Hash}
   */

  getStandardTemplateHash(index) {
    ...
    if (this._hashSequence) {
      ...
      const hashSequence = blake2b.digest(bw.render());

      if (!this.mutable)
        this._hashSequence = hashSequence;

      sequences = hashSequence;
    }
    ...
    if (this._hashOutputs) {
      ...
      const hashOutputs = blake2b.digest(bw.render());
    ...
    const bw = bio.pool(84);

    bw.writeU32(this.version);
    bw.writeU32(this.locktime);
    bw.writeU32(this.inputs.length);
    bw.writeBytes(sequences);
