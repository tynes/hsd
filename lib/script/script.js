  ...
  execute(stack, flags, tx, index, value) {
    ...
    for (let ip = 0; ip < this.code.length; ip++) {
      ...
      switch (op.value) {
        ...
        case opcodes.OP_CHECKTEMPLATEVERIFY: {
          if (!(flags & Script.flags.VERIFY_STANDARD_TEMPLATE))
            break;
          ...
          switch (item.length) {
            case 32: {
              ...
              const hash = tx.getStandardTemplateHash(index);

              if (!item.equals(hash))
                throw new ScriptError('CHECKTEMPLATEVERIFY', op, ip);
