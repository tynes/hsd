      ...
      if (this.chain.tip.hash.equals(tip.hash)) {
        ...
        if (await this.chain.isMainChain(tip)) {
          ...
          if ((tip.height < post) && (tip.height >= pre)) {
            ...
            const txn = this.chain.db.tree.txn();

            // post can't be greater than the tip
            let height = post;
            ...
            while (height-- > fork.height) {
              ...
              const undo = await this.chain.getNameUndo(entry.hash);
              ...
              for (const [nameHash, delta] of undo.names) {
                const ns = await view.getNameState(this.chain.db, nameHash);
            ...
            while (!fork.hash.equals(entry.hash)) {
              const block = await this.chain.getBlock(entry.hash);
              const view = await this.chain.getBlockView(block);
              ...
              if (undo) {
                for (const [nameHash] of undo.names) {
                  const ns = await view.getNameState(this.chain.db, nameHash);
                  view.addName(nameHash, ns);
                }
              }
            ...
            root = await txn.hash;
            console.log(`calc: ${root.toString('hex')}`)
          }
        }
      }
    }
