...
class Miner extends EventEmitter {
  ...
  async _createBlock(tip, address) {
    ...
    // treeRoot should always be the correct value
    // when there is no tip passed in
    //if (tip.height % this.network.names.treeInterval === 1) {}


    let root;
    if (!tip) {
      tip = this.chain.tip;
      root = this.chain.db.treeRoot();
    } else {
      // use the db tree root when mining on the chain tip
      if (this.chain.tip.hash.equals(tip.hash)) {
        root = this.chain.db.treeRoot();
      } else {

        // its in the main chain
        if (await this.chain.isMainChain(tip)) {
          // next block needs to be updated
          if ((tip.height % treeInterval) === 0) {
            // fetch main chain block at next height and use that tree root
            const entry = await this.chain.getEntryByHeight(tip.height + 1);
            root = entry.treeRoot;
          } else {
            root = tip.treeRoot;
          }
        } else {
          const fork = await this.chain.findFork(tip, this.chain.tip);

          const post = fork.height + (treeInterval - (fork.height % treeInterval));
          const pre = fork.height - (fork.height % treeInterval);

          // use the fork's tree root if in the same interval
          if ((tip.height <= post) && (tip.height >= pre)) {
            root = fork.treeRoot;
          } else {
            // need to calculate the tree root
            // instead of cloning the txn, create a
            // new txn based on chain.db.treeRoot and
            // then navigate from there
            const txn = this.txn.clone();
            let height = this.chain.tip.height;

            // get all of the disconnects
              // remove each name from the txn

            // then get all of the connects
              // add each name to the txn

            const disconnects = [];
            let entry = this.chain.tip;
            while (height > fork.height) {
              entry = this.chain.getPrevious(entry);
              disconnects.push(entry);
              height--;
            }

            // NEW WORK

            // add an additional field to TRANSFER
            // covenant items
            //   address and rcvaddress
            // for finalize, the output must match
            // the rcvaddress
            // for update, the output must match
            // the address
            // spending to TRANSFER
            //   the covenant items must include
            //   address and rcvaddress
            // the address must match the address the
            // transfer came from
            //
            // spending to TRANSFER
            //   covenant items
            //    [hash, height, version, addr, recvaddr]
            //   addr must match the TRANSFER output addr
            //   addr must be the UTXO being consumed address
            //
            // spending from TRANSFER
            // to UPDATE (cancel)
            //   output address must match covenant addr
            //   how does the cancel work for now?
            //   it must take the addr out of the prev UTXO?
            //   how does REVOKE work here?
            //
            // to FINALIZE
            //   output address must match covenant recvaddr
            //
            // how exactly does REVOKE work?
            //
            // WOW, explore:
            // use the script that is in the covenant
            // as redeem in the vm

            // new sighash type to null out the final
            // item in the covenant items
            // OR
            // new sighash type to null out the 4th item
            //
            // need to add additional items to the covenant items
            // to make sure it doesn't break certain things


            // need to calculate the treeRoot...
            // update tip.treeRoot as txn is committed to
            // but this.chain.db.txn could be different
            // snapshot the txn and remove until appropriate
            // for each block
            //   for each nameHash
            //     this.txn.remove(nameHash)
            // https://github.com/handshake-org/hsd/blob/e1c0281dd9e9bbbd23f5cd832c82ebe3808e9b4f/lib/blockchain/chaindb.js#L1871
            // Should follow chaindb.disconnectNames
            // and chaindb.saveNames
          }
        }
      }
    }
    ...
    //let root = this.chain.db.treeRoot();
