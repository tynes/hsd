...
class Peer extends EventEmitter {
  ...
  constructor(options) {
    ...
    this.window = new SlidingWindow({limit: options.p2prps});
    this.init();
  }
  ...
  sendProof(root, key, proof) {
    ...
    this.window.increase(1);
    if (!this.window.allow()) {
      this.logger.debug('sendproof: rate limit exceeded (%s).', this.hostname());
      this.ban();
      return;
    }
