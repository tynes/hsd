...
class HTTP extends Server {
  ...
  initRouter() {
    ...
    // Wallet Names/Auctions
    this.get('/wallet/:id/names', async (req, res) => {
      const height = this.wdb.height;
      const network = this.network;
    ...
    this.post('/wallet/:id/transfer', async (req, res) => {
      ...
      const options = TransactionOptions.fromValidator(valid);
      const mtx = await req.wallet.createTransfer(name, address, options);
...
class TransactionOptions {
  ...
  static fromValidator(valid) {
    return new TransactionOptions().fromValidator(valid);
  }
}
