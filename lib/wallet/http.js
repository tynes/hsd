...
class HTTP extends Server {
  ...
  initSockets() {
    ...
    const handleCovenant = (event, wallet, data, details) => {
      const name = `w:${wallet.id}`;
      ...
      if (this.channel(name))
        this.to(name, event, wallet.id, data, json);

      if (this.channel('w:*'))
        this.to('w:*', event, wallet.id, data, json);
    };
    ...
    for (const type of Object.values(rules.typesByVal))
      if (type !== 'NONE') {
        ...
        this.wdb.on(channel, (wallet, data, details) => {
          handleCovenant(type, wallet, data, details);
        });
      }
  }
