...
class Wallet extends EventEmitter {
  ...
  async makeOpen(name, force, acct, outputs) {
    assert(typeof name === 'string');
  ...
  async _createOpen(name, force, options) {
    ...
    const outputs = options ? options.outputs || [] : [];
    const mtx = await this.makeOpen(name, force, acct, outputs);
  ...
  /**
   * Add outputs to a transaction and checks to see if the output
   * is dust or if value is being sent to an unknown/null address.
   * @param {MTX} mtx
   * @param {Object[]} outputs
   * @returns {MTX}
   */

  addOutputs(mtx, outputs) {
