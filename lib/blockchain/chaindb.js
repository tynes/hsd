...
class ChainDB {
  ...
  async getBlockView(block) {
    ...
    const nameundo = await this.getNameUndo(block.hash());
  ...
  async saveNames(view, height) {
    ...
    if ((height % this.network.names.treeInterval) === 0) {
      const commit = await this.txn.commit();
      const prev = await this.db.get(layout.s.encode());
      console.log(`height: ${height}`);
      console.log(`old: ${prev ? prev.toString('hex') : null}`);
      console.log(`new: ${commit.toString('hex')}`);
      this.put(layout.s.encode(), commit);
    }
  }
  ...
  async connectNames(view, height) {
    ...
    if (undo.names.length === 0) {
      ...
      //console.log(`delete undo: ${height}`);
    } else {
      this.put(layout.w.encode(height), undo.encode());
      console.log(`putting undo: ${height}`);
    }
  ...
  /**
   * Get NameUndo by height.
   * @param {Hash|Number} block - Hash or height.
   * @returns {Promise} - Returns {NameUndo}
   */

  async getNameUndo(block) {
    if (Buffer.isBuffer(block)) {
      const entry = await this.getEntryByHash(block);
      if (!entry)
        return null;
      block = entry.height;
    }
    ...
    const raw = await this.db.get(layout.w.encode(block));
