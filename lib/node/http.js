...
class HTTP extends Server {
  ...
  initRouter() {
    ...
    this.get('/name/hash/:hash', async (req, res) => {
      ...
      enforce(hash, 'Must provide hash');
    ...
    // Get name resource
    this.get('/name/:name/resource', async (req, res) => {
      const valid = Validator.fromRequest(req);
      const name = valid.str('name');

      enforce(name && rules.verifyName(name), 'Invalid name.');
      ...
      try {
        res.json(200, {
          record: resource.toJSON()
        });
      } catch (e) {
        res.json(200, {
          record: resource.toHex()
        });
      }
    });

    this.get('/name/:name/proof', async (req, res) => {
      const valid = Validator.fromRequest(req);
      const name = valid.str('name');
      const treeRoot = valid.bhash('root');
    ...
    this.get('/name/grind/:size', async (req, res) => {
      const valid = Validator.fromRequest(req);
      const size = valid.u32('size', 10);

      enforce(size > 0 && size < 64, 'Invalid length.');

      const network = this.network;
      const height = this.chain.height;

      const name = rules.grindName(size, height + 1, network);
