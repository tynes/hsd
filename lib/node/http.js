...
class HTTP extends Server {
  ...
  initRouter() {
    ...
    // Get undo coins/names by hash/height
    this.get('/undo/:hash', async (req, res) => {
      const valid = Validator.fromRequest(req);
      ...
      const undo = await this.chain.getUndoCoins(hash);
      const nameundo = await this.chain.getNameUndo(hash);

      const coins = [];
      for (const coin of undo.items.values()) {
        coins.push(coin);
      }

      const names = {};
      for (const [nameHash, ns] of nameundo.names.values()) {
