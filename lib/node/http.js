...
class HTTP extends Server {
  ...
  initRouter() {
    ...
    this.get('/dns-query', async (req, res) => {
      const valid = Validator.fromRequest(req);
      const name = valid.str('name');
      const type = valid.uint('type');

      if (!name || !type) {
        res.json(400, {message: 'Must pass name and type'});
        return;
      }

      const msg = await this.node.rs.lookup(name, type);
