...
class HTTP extends Server {
  ...
  initRouter() {
    ...
    // Proof of Work details by hash/height
    this.get('/pow/:block', async (req, res) => {
      const valid = Validator.fromRequest(req);
      const hash = valid.uintbhash('block');
      ...
      res.json(200, {
        preHead: header.toPrehead().toString('hex'),
        subHead: header.toSubhead().toString('hex'),
        subHash: header.subHash().toString('hex'),
        maskHash: header.maskHash().toString('hex'),
        pad32: header.padding(32).toString('hex'),
        pad20: header.padding(20).toString('hex'),
        pad8: header.padding(8).toString('hex'),
        commitHash: header.commitHash().toString('hex'),
        shareHash: header.shareHash().toString('hex'),
        powHash: header.powHash().toString('hex')
      });
    });
