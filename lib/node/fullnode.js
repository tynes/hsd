...
const {AIRDROP_ROOT, FAUCET_ROOT} = require('../primitives/airdropproof');
...
class FullNode extends Node {
  ...
  async open() {
    ...
    // Integrity check
    const genesis = await this.chain.getBlock(0);
    const cbWitness = genesis.txs[0].inputs[0].witness;
    assert(cbWitness.items[2].equals(AIRDROP_ROOT),
      'Integrity check failed for AIRDROP_ROOT');
    assert(cbWitness.items[3].equals(FAUCET_ROOT),
      'Integrity check failed for FAUCET_ROOT');
