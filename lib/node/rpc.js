...
class RPC extends RPCBase {
  ...
  init() {
    ...
    this.add('getrawmempoolorphans', this.getRawMempoolOrphans);
  ...
  // TODO: here
  async getInvalidBlocks(args, help) {
    if (help || args.length > 0)
      throw new RPCError(errs.MISC_ERROR, 'getinvalidblocks');
  ...
  async getRawMempoolOrphans(args, help) {
    if (help || args.length > 1)
      throw new RPCError(errs.MISC_ERROR, 'getrawmempoolorphans ( verbose )');
    ...
    if (verbose) {
      ...
      for (const orphan of this.mempool.orphans.values()) {
        const tx = entry.toTX();
        out[tx.txid()] = tx.toJSON();
      }

      return out;
    }

    const hashes = this.mempool.getOrphanSnapshot();
